...
class DAXFlash(metaclass=LogBase):
    ...
    def readflash(self, addr, length, filename, parttype=None, display=True) -> (bytes, bool):
        ...
        try:
            ...
            if error_occurred and bytesread > 0:
                self.warning(f"Partial read: {hex(bytesread)} of {hex(length)} bytes saved to file")
            if filename and rq is not None:
                try:
                    rq.put(None, timeout=5)
                except Exception:
                    pass
        ...
        if filename:
            if error_occurred and bytesread > 0:
                return True  # partial data was saved
    ...
    def writeflash(self, addr, length, filename: str = "", offset=0, parttype=None, wdata=None, display=True):
        ...
        plen = self.get_packet_length()
        write_packet_size = plen.write_packet_length if plen else 0x100000
