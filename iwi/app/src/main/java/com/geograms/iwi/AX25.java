...
public class AX25 {
    ...
    public static final int TX_AMPLITUDE = 24000;
    ...
    public static byte[] encodeAddress(String callsign, int ssid, boolean isLast) {
        ...
        // SSID byte: 0CRRSSID  C=command/response, RR=11 (reserved)
        // isLast sets the extension bit (bit 0)
    ...
    public static byte[] buildUIFrame(String src, int srcSsid, String dst, int dstSsid,
        ...
        // Destination address
        boolean lastAddr = (digiCount == 0);
        byte[] dstAddr = encodeAddress(dst, dstSsid, lastAddr && (digiCount == 0 ? true : false));
        // Source is last only if there are no digipeaters
        // Actually: dst is never last unless src is also absent, which doesn't happen.
        // dst is first, src is second, digipeaters follow. Last address has extension bit set.
        byte[] dstBytes = encodeAddress(dst, dstSsid, false);
        System.arraycopy(dstBytes, 0, frame, pos, 7);
        pos += 7;

        boolean srcIsLast = (digiCount == 0);
        byte[] srcBytes = encodeAddress(src, srcSsid, srcIsLast);
    ...
    public static short[] modulateAfsk(boolean[] tones) {
        ...
        for (int bitIdx = 0; bitIdx < tones.length; bitIdx++) {
            ...
            double bitEnd = (bitIdx + 1) * SAMPLES_PER_BIT;
            while (sampleIdx < bitEnd && sampleIdx < pcm.length) {
                pcm[sampleIdx] = (short) (TX_AMPLITUDE * Math.sin(phase));
