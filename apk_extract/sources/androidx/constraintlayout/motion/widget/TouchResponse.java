...
class TouchResponse {
    ...
    private void fill(TypedArray typedArray) {
        ...
        for (int i2 = 0; i2 < indexCount; i2++) {
            ...
            if (index == R.styleable.OnSwipe_touchAnchorId) {
                ...
                this.mTouchSide = i4;
                float[] fArr2 = TOUCH_DIRECTION[i4];
    ...
    public void processTouchEvent(MotionEvent motionEvent, MotionLayout.MotionTracker motionTracker, int i2, MotionScene motionScene) {
        ...
        if (action == 1) {
            ...
            if (f4 == 0.0f || f4 == 1.0f || (i3 = this.mOnTouchUp) == 3) {
                if (0.0f >= f4 || 1.0f <= f4) {
            ...
            this.mMotionLayout.touchAnimateTo(i3, ((double) f4) < 0.5d ? 0.0f : 1.0f, f3);
        ...
        if (Math.abs((this.mTouchDirectionY * rawY) + (this.mTouchDirectionX * rawX)) > this.mDragThreshold || this.mDragStarted) {
            ...
            if (Math.abs(((this.mTouchDirectionY * fArr4[1]) + (f5 * fArr4[0])) * this.mDragScale) < 0.01d) {
    ...
    public void setupTouch() {
        ...
        if (viewFindViewById instanceof NestedScrollView) {
            ...
            nestedScrollView.setOnScrollChangeListener(new k() { // from class: androidx.constraintlayout.motion.widget.TouchResponse.2
                @Override // u.k
