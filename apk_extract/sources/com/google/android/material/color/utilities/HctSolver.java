...
public class HctSolver {
    ...
    public static boolean areInCyclicOrder(double d2, double d3, double d4) {
        return sanitizeRadians(d3 - d2) < sanitizeRadians(d4 - d2);
    }

    public static double[] bisectToLimit(double d2, double d3) {
        int iCriticalPlaneAbove;
        int iCriticalPlaneBelow;
        double[][] dArrBisectToSegment = bisectToSegment(d2, d3);
        ...
        for (int i2 = 0; i2 < 3; i2++) {
            ...
            if (d4 != d5) {
                if (d4 < d5) {
                    iCriticalPlaneAbove = criticalPlaneBelow(trueDelinearized(d4));
                    iCriticalPlaneBelow = criticalPlaneAbove(trueDelinearized(dArr2[i2]));
                } else {
                    iCriticalPlaneAbove = criticalPlaneAbove(trueDelinearized(d4));
                    iCriticalPlaneBelow = criticalPlaneBelow(trueDelinearized(dArr2[i2]));
                }
                int i3 = iCriticalPlaneAbove;
                int i4 = iCriticalPlaneBelow;
                for (int i5 = 0; i5 < 8 && Math.abs(i4 - i3) > 1; i5++) {
                    int iFloor = (int) Math.floor((i3 + i4) / 2.0d);
                    double[] coordinate = setCoordinate(dArr, CRITICAL_PLANES[iFloor], dArr2, i2);
                    double dHueOf2 = hueOf(coordinate);
                    if (areInCyclicOrder(dHueOf, d3, dHueOf2)) {
                        dArr2 = coordinate;
                        i4 = iFloor;
                    } else {
                        dArr = coordinate;
    ...
    public static double[][] bisectToSegment(double d2, double d3) {
        ...
        for (int i2 = 0; i2 < 12; i2++) {
            double[] dArrNthVertex = nthVertex(d2, i2);
            if (dArrNthVertex[0] >= 0.0d) {
                ...
                if (!z2) {
                    ...
                    d4 = dHueOf;
                    d5 = d4;
                } else if (z3 || areInCyclicOrder(d4, dHueOf, d5)) {
                    if (areInCyclicOrder(d4, d3, dHueOf)) {
                        z3 = false;
                        dArr2 = dArrNthVertex;
                        d5 = dHueOf;
                    } else {
    ...
    public static double chromaticAdaptation(double d2) {
        ...
        return ((MathUtils.signum(d2) * 400.0d) * dPow) / (dPow + 27.13d);
    }
    ...
    public static int findResultByJ(double d2, double d3, double d4) {
        double dSqrt = Math.sqrt(d4) * 11.0d;
        ViewingConditions viewingConditions = ViewingConditions.DEFAULT;
        double dPow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d);
        double ncb = viewingConditions.getNcb() * viewingConditions.getNc() * (Math.cos(d2 + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d;
        ...
        while (i2 < 5) {
            ...
            double d6 = dPow;
            double dPow2 = Math.pow(((d3 == 0.0d || dSqrt == 0.0d) ? 0.0d : d3 / Math.sqrt(d5)) * dPow, 1.1111111111111112d);
            double d7 = dSqrt;
            double dPow3 = (Math.pow(d5, (1.0d / viewingConditions.getC()) / viewingConditions.getZ()) * viewingConditions.getAw()) / viewingConditions.getNbb();
            double d8 = (((0.305d + dPow3) * 23.0d) * dPow2) / (((dPow2 * 108.0d) * dSin) + (((dPow2 * 11.0d) * dCos) + (23.0d * ncb)));
            double d9 = d8 * dCos;
            double d10 = d8 * dSin;
            double d11 = dPow3 * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((288.0d * d10) + ((451.0d * d9) + d11)) / 1403.0d), inverseChromaticAdaptation(((d11 - (891.0d * d9)) - (261.0d * d10)) / 1403.0d), inverseChromaticAdaptation(((d11 - (d9 * 220.0d)) - (d10 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d12 = dArrMatrixMultiply[0];
            if (d12 < 0.0d) {
                break;
            }
            double d13 = dArrMatrixMultiply[1];
            if (d13 < 0.0d) {
                break;
            }
            double d14 = dArrMatrixMultiply[2];
            if (d14 < 0.0d) {
                break;
            }
            double[] dArr = Y_FROM_LINRGB;
            double d15 = (dArr[2] * d14) + (dArr[1] * d13) + (dArr[0] * d12);
            if (d15 <= 0.0d) {
                return 0;
            }
            if (i2 != 4) {
                double d16 = d15 - d4;
                if (Math.abs(d16) >= 0.002d) {
                    dSqrt = d7 - ((d16 * d7) / (d15 * 2.0d));
                    i2++;
                    dPow = d6;
                }
            }
    ...
    public static double intercept(double d2, double d3, double d4) {
        return (d3 - d2) / (d4 - d2);
    }

    public static double inverseChromaticAdaptation(double d2) {
        double dAbs = Math.abs(d2);
        return Math.pow(Math.max(0.0d, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d) * MathUtils.signum(d2);
    }
    ...
    public static double[] lerpPoint(double[] dArr, double d2, double[] dArr2) {
        double d3 = dArr[0];
        double d4 = dArr[1];
        double d5 = dArr[2];
        return new double[]{((dArr2[0] - d3) * d2) + d3, ((dArr2[1] - d4) * d2) + d4, ((dArr2[2] - d5) * d2) + d5};
    }
    ...
    public static double[] nthVertex(double d2, int i2) {
        double[] dArr = Y_FROM_LINRGB;
        double d3 = dArr[0];
        double d4 = dArr[1];
        double d5 = dArr[2];
        double d6 = i2 % 4 <= 1 ? 0.0d : 100.0d;
        double d7 = i2 % 2 == 0 ? 0.0d : 100.0d;
        if (i2 < 4) {
            double d8 = ((d2 - (d4 * d6)) - (d5 * d7)) / d3;
            return isBounded(d8) ? new double[]{d8, d6, d7} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i2 < 8) {
            double d9 = ((d2 - (d3 * d7)) - (d5 * d6)) / d4;
            return isBounded(d9) ? new double[]{d7, d9, d6} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d10 = ((d2 - (d3 * d6)) - (d4 * d7)) / d5;
        return isBounded(d10) ? new double[]{d6, d7, d10} : new double[]{-1.0d, -1.0d, -1.0d};
    }
    ...
    public static double[] setCoordinate(double[] dArr, double d2, double[] dArr2, int i2) {
        return lerpPoint(dArr, intercept(dArr[i2], d2, dArr2[i2]), dArr2);
    }

    public static Cam16 solveToCam(double d2, double d3, double d4) {
        return Cam16.fromInt(solveToInt(d2, d3, d4));
    }

    public static int solveToInt(double d2, double d3, double d4) {
        if (d3 < 1.0E-4d || d4 < 1.0E-4d || d4 > 99.9999d) {
            return ColorUtils.argbFromLstar(d4);
        }
        double dSanitizeDegreesDouble = (MathUtils.sanitizeDegreesDouble(d2) / 180.0d) * 3.141592653589793d;
        double dYFromLstar = ColorUtils.yFromLstar(d4);
        int iFindResultByJ = findResultByJ(dSanitizeDegreesDouble, d3, dYFromLstar);
