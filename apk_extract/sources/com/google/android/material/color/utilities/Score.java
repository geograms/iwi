...
public final class Score {
    ...
    public static List<Integer> score(Map<Integer, Integer> map) {
        ...
        for (Map.Entry entry : map2.entrySet()) {
            ...
            double d2 = 0.0d;
            for (int i2 = iRound2 - 15; i2 < iRound2 + 15; i2++) {
                d2 += dArr[MathUtils.sanitizeDegreesInt(i2)];
            }
            map3.put(Integer.valueOf(iIntValue2), Double.valueOf(d2));
        }
        HashMap map4 = new HashMap();
        for (Map.Entry entry2 : map2.entrySet()) {
            int iIntValue3 = ((Integer) entry2.getKey()).intValue();
            Cam16 cam16 = (Cam16) entry2.getValue();
            map4.put(Integer.valueOf(iIntValue3), Double.valueOf(((cam16.getChroma() - TARGET_CHROMA) * (cam16.getChroma() < TARGET_CHROMA ? WEIGHT_CHROMA_BELOW : WEIGHT_CHROMA_ABOVE)) + (((Double) map3.get(Integer.valueOf(iIntValue3))).doubleValue() * 100.0d * WEIGHT_PROPORTION)));
        }
        ...
        while (it3.hasNext()) {
            ...
            while (true) {
                ...
                if (MathUtils.differenceDegrees(cam162.getHue(), ((Cam16) map2.get((Integer) it4.next())).getHue()) < CUTOFF_CHROMA) {
                    break;
                }
            }
        }
